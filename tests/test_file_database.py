...
class TestFileDatabase(unittest.TestCase):
    def test_data_is_saved_between_instances(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first_db.create_table("movies", ("movie_id", "title", "year", "genre", "rating"))
            first_db.insert_record(
                "movies",
                {"movie_id": 1, "title": "Матрица", "year": 1999, "genre": "фантастика", "rating": 9.5},
            )
    ...
    def test_select_with_filters(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            db.insert_record("movies", {"movie_id": 1, "title": "Матрица", "year": 1999, "genre": "фантастика", "rating": 9.5})
            db.insert_record("movies", {"movie_id": 2, "title": "Интерстеллар", "year": 2014, "genre": "фантастика", "rating": 9.0})

            records = db.select_records("movies", title="Интерстеллар")

            self.assertEqual(
                records,
                [{"movie_id": 2, "title": "Интерстеллар", "year": 2014, "genre": "фантастика", "rating": 9.0}],
            )
